...
import { StockPositionReturns } from "@/helpers/stockPositionHelper";
...
export const StockPositionComponent = (props: Props) => {
  ...
  const formattedPercent =
    (returnsAsPercent > 0 ? "+" : "") +
    (returnsAsPercent * 100).toFixed(2) +
    "%";
  const returnsStyle =
    returnsAsPercent > 0
      ? styles.positive_returns
      : returnsAsPercent < 0
      ? styles.negative_returns
      : "";
        ...
        <table className={styles.position_information}>
          ...
          <tbody>
            <tr>
              ...
              <td className={styles.table_entry}>
                <span className={returnsStyle}>${returnsAsAmount}</span>
              </td>
              <td className={styles.table_entry}>
                <span className={returnsStyle}>{formattedPercent}</span>
              </td>
